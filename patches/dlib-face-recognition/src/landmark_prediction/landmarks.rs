...
cpp_class!(
    /// A wrapper around the dlib `full_object_detection` class, which internally has a `std::vector<dpoint>`.
...
impl Deref for FaceLandmarks {
    ...
    fn deref(&self) -> &Self::Target {
        ...
        if len == 0 {
            ...
            unsafe {
                // dlib 20.x stores dpoint (double) internally — part(0) is the first element
                let pointer = cpp!([self as "dlib::full_object_detection*"] -> *const Point as "dlib::dpoint*" {
                    return &self->part(0);
                });
